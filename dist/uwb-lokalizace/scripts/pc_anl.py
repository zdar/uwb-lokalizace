...
UDP_PORT = 50000
RAW_RPT_FORWARD_PORT = 50001   # (UDP) forward raw RPT packets for qr_scanner.py
RAW_RPT_FORWARD_ADDR = ("127.0.0.1", RAW_RPT_FORWARD_PORT)
QR_EVENT_PORT = 50002          # receive QR scan events from ESP32-CAM
HTTP_PORT = 5000
# (TCP) HTTPS mirror so remote devices get a secure context for the camera.
# Same number as RAW_RPT_FORWARD_PORT is fine: that one is UDP, this one is TCP.
HTTPS_PORT = 50001
...
auto_cal = {
    ...
    "collect_start_ms": 0,    # when the first RPT arrived and collection started
...
def ensure_session():
    ...
    if solved_path and _file_age_hours(solved_path) < 2:
        ...
        log(f"[SESSION] Reusing recent session (<2h old): raw={raw_path}, solved={solved_path}")
        return session_csv_raw, session_csv_solved

    return new_session()
qr_last_seen_ms = None        # last QR event timestamp from ESP32-CAM
qr_detected_ms = None         # timestamp of the last actual QR code scan
qr_detected_code = None       # last QR code detected by webcam (detect-only mode)
qr_previous_code = None       # previous QR code detected by webcam
...
def finish_qr_collection(collection):
    ...
    ts = datetime.now().isoformat()
    duration_ms = time.time() * 1000 - collection["start_ms"]
    ensure_session()
    ...
    log(f"[QR] Saved {qr_code}: duration={duration_ms/1000:.2f}s, ranges={len(median_ranges)}, computed={computed_xyz}, global={global_xyz}")
...
# ---------------------------------------------------------------------------
# HTTPS (self-signed) support for remote camera access
# ---------------------------------------------------------------------------

CERT_DIR = os.path.join(PROJECT_ROOT, "certs")
CERT_FILE = os.path.join(CERT_DIR, "https_cert.pem")
CERT_KEY_FILE = os.path.join(CERT_DIR, "https_key.pem")


def ensure_https_cert():
    """Create a self-signed certificate in CERT_DIR on first run.

    Returns (cert_path, key_path), or None when HTTPS cannot be enabled
    (missing 'cryptography' package or generation failure)."""
    ...
    try:
        ...
        log("HTTPS disabled: 'cryptography' package is missing (run setup.bat)")
        return None
    try:
        ...
        try:
            ...
            pass
        san = [x509.DNSName(n) for n in sorted(names) if n]
        san += [x509.IPAddress(ipaddress.ip_address(ip)) for ip in sorted(ips)]
        subj = x509.Name([
            x509.NameAttribute(NameOID.COMMON_NAME, "uwb-lokalizace"),
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "uwb-lokalizace"),
        ])
        ...
        log(f"Generated self-signed HTTPS certificate in {CERT_DIR} "
            f"(SANs: {', '.join(sorted(names) + sorted(ips))})")
        return CERT_FILE, CERT_KEY_FILE
    except Exception as e:
        log(f"HTTPS disabled: certificate generation failed: {e}")
        return None


def start_https_server():
    """Serve the same Flask app over HTTPS in a background thread.

    Returns True when the HTTPS listener was started."""
    pair = ensure_https_cert()
    if not pair:
        return False
    ...
    def _serve():
        run_simple("0.0.0.0", HTTPS_PORT, app,
                   ssl_context=pair, threaded=True, use_reloader=False)
    ...
    with auto_cal_lock:
        if (auto_cal["running"] and
            ...
            if auto_cal["phase"] == "wait_tag":
                ...
                auto_cal["collect_start_ms"] = now_ms
            ...
            with qr_collect_lock:
                for coll in qr_active:
                    if coll.get("tag_id") == tid:
                        ...
                        break
        # Only persist raw packets while an active collection is running.
        # This prevents the raw CSV from growing indefinitely with untriggered traffic.
        if comment or trigger:
            ts_ms = int(now)
...
def qr_listener():
    ...
    global qr_last_seen_ms, qr_detected_ms, qr_detected_code, qr_previous_code
    global esp32_cam_ip, esp32_cam_last_seen_ms
    ...
    while True:
        ...
        elif text.startswith("QR_DETECT,"):
            # Webcam detect-only mode: show the code in the GUI but do not start
            # a collection. The user triggers sampling manually from the UI.
            qr_last_seen_ms = time.time() * 1000
            qr_code = text[10:].strip()
            if qr_code:
                if qr_detected_code and qr_detected_code != qr_code:
                    qr_previous_code = qr_detected_code
                qr_detected_code = qr_code
                qr_detected_ms = time.time() * 1000
                log(f"[QR_DETECT] webcam detected {qr_code}")
        elif text == "HB,CAM":
            esp32_cam_ip = addr[0]
    ...
    while True:
        ...
        if phase == "collecting":
            ...
            with auto_cal_lock:
                ...
                collect_start_ms = auto_cal["collect_start_ms"]
            ...
            collect_duration_ms = time.time() * 1000 - collect_start_ms
            log(f"[AUTO] Collected {actual_packets} packets from tag {current_id} in {collect_duration_ms/1000:.2f}s, median ranges: {median_ranges}")
...
<head>
    ...
    <style>
        ...
        #measureModal { display: none; overflow-y: auto; text-align: left; padding-top: 150px; }
        #measureModal .nav-bar { text-align: center; margin: 0 0 15px 0; }
        .measure-content { display: flex; max-width: 1400px; margin: 80px auto 0 auto; padding: 0 20px 400px 20px; gap: 50px; }
        #measureTransformInfo { text-align: center; }
        #measureTagPositions { text-align: center; }
        #measureModal .dev-link { text-align: center; }
        .measure-left { width: 30%; min-width: 300px; flex-shrink: 0; }
        .measure-right { width: 70%; padding-left: 60px; text-align: center; }
        .measure-right h1 { text-align: center; margin-bottom: 5px; }
        .measure-right h2 { text-align: center; }
        .measure-right .row { text-align: center; }
        .measure-right table { margin: 0 auto; text-align: left; }
        .measure-session-path { font-size: 13px; color: #666; margin: 0 0 20px 0; word-break: break-all; text-align: center; }
        #measureModal .status { font-size: 20px; margin: 10px 0; }
        ...
        .nav-bar { margin: 50px 0 15px 0; }
        .nav-bar button { padding: 6px 12px; font-size: 13px; margin: 0 3px; }
        .webcam-container { position: relative; display: inline-block; margin: 10px 0; max-width: 100%; }
        .webcam-container video { display: block; max-width: 100%; height: auto; }
        .webcam-container canvas { position: absolute; top: 0; left: 0; width: 100%; height: 100%; }
...
<body>
    ...
    <div id="measureModal" class="modal">
        ...
        <div class="measure-content">
            <div class="measure-left">
                ...
                <div id="webcamQrPrevious" style="font-size:14px; color:#666; min-height:20px; margin-top:4px;"></div>
                <div style="margin-top:8px;">
                    <button id="webcamQrBtn" onclick="webcamQrTrigger()" disabled>Spustit měření s naskenovaným kódem</button>
                </div>
                <div id="webcamQrStatus" style="margin-top:8px; font-weight:bold; color:#333; min-height:24px;"></div>
            </div>
            <div class="measure-right">
                <h1>Měření</h1>
                <div id="measureSessionPath" class="measure-session-path">-</div>
    ...
    <script>
        ...
        function showMeasureModal() {
            ...
            document.getElementById('mainContent').style.display = 'none';
            pollMeasureState();
            // Auto-start the browser webcam when entering the measurement page.
            setTimeout(() => startWebcamScanning(), 300);
        }
        ...
        let lastWebcamQrCode = null;

        function webcamQrTrigger() {
            const qr = lastWebcamQrCode;
            if (!qr) return;

            const btn = document.getElementById('webcamQrBtn');
            const status = document.getElementById('webcamQrStatus');
            btn.disabled = true;
            ...
            fetch('/manual_qr', {
                ...
                // 5..1 countdown with beeps, matching the camera QR routine.
                let count = 5;
                status.textContent = `Měření ${qr}: ${count}`;
                playTone(880, 0.1);
                ...
                const timer = setInterval(() => {
                    ...
                    if (count > 0) {
                        ...
                        clearInterval(timer);
                        status.textContent = `Měření ${qr}: dokončeno`;
                        playScanSuccessSound();
                        setTimeout(() => {
                            status.textContent = '';
                            btn.disabled = false;
        ...
        // -----------------------------------------------------------------
        // In-browser webcam QR scanner (runs inside the Měření page)
        // -----------------------------------------------------------------
        let webcamStream = null;
        let webcamScanInterval = null;
        let webcamCanvasCtx = null;
        let webcamLastCode = null;
        let webcamLastReportMs = 0;
        ...
        async function startWebcamScanning() {
            ...
            try {
                ...
                webcamCanvasCtx = canvas.getContext('2d');
                startBtn.disabled = true;
                stopBtn.disabled = false;

                // Match canvas size to video once it starts playing.
        ...
        function stopWebcamScanning() {
            ...
            webcamCanvasCtx = null;
        }

        function processWebcamFrame() {
            const video = document.getElementById('webcamVideo');
            const canvas = document.getElementById('webcamOverlay');
            const ctx = webcamCanvasCtx;
            if (!ctx || video.readyState !== video.HAVE_ENOUGH_DATA) return;

            // Draw the HUD on the overlay canvas.
            ctx.clearRect(0, 0, canvas.width, canvas.height);

            // Prepare a small offscreen canvas for jsQR.
            ...
            if (code && code.data) {
                ...
                // Throttle reporting of the same code.
                if (detectedText !== webcamLastCode || nowMs - webcamLastReportMs >= WEBCAM_REPEAT_MS) {
                    webcamLastCode = detectedText;
                    webcamLastReportMs = nowMs;
                    reportWebcamQr(detectedText);
                    playWebcamBeep(1500, 150);
                }
                ...
                ctx.lineWidth = 4;
                ctx.beginPath();
                ctx.moveTo(rect.topLeftCorner.x, rect.topLeftCorner.y);
                ctx.lineTo(rect.topRightCorner.x, rect.topRightCorner.y);
                ctx.lineTo(rect.bottomRightCorner.x, rect.bottomRightCorner.y);
                ctx.lineTo(rect.bottomLeftCorner.x, rect.bottomLeftCorner.y);
                ctx.closePath();
                ctx.stroke();

                ctx.fillStyle = '#00ff00';
                ctx.font = 'bold 18px sans-serif';
                ctx.fillText(detectedText, rect.topLeftCorner.x, rect.topLeftCorner.y - 8);
            }

            drawWebcamHud(ctx, canvas.width, canvas.height, detectedText);
        }

        function drawWebcamHud(ctx, w, h, detectedText) {
            // Top status bar.
            ctx.fillStyle = 'rgba(0, 0, 0, 0.7)';
            ctx.fillRect(0, 0, w, 34);
            ctx.fillStyle = detectedText ? '#00ff00' : '#ffff00';
            ctx.font = 'bold 16px sans-serif';
            const statusText = detectedText ? `DETEKOVANO: ${detectedText}` : 'NASKENUJTE QR KOD';
            ctx.fillText(statusText, 10, 24);
            ...
            const prevText = webcamPreviousCode ? `Predchozi: ${webcamPreviousCode}` : 'Predchozi: -';
            ctx.fillText(prevText, 10, 56);
        }

        function reportWebcamQr(qr) {
            fetch('/qr_detect', {
                method: 'POST',
                headers: {'Content-Type': 'application/json'},
                body: JSON.stringify({qr: qr})
            }).catch(err => console.error('qr_detect failed:', err));
        }

        function playWebcamBeep(freq, durationMs) {
            try {
        ...
        function renderMeasureState() {
            Promise.all([
                ...
                const videoActive = webcamStream && webcamStream.active;
                if (videoActive) {
                    camEl.innerHTML = '<span style="color:green">Webkamera aktivní</span>';
                } else {
                    camEl.innerHTML = '<span style="color:orange">Webkamera neaktivní</span> - klikni "Zapnout kameru" nebo přejdi zpět a znovu na Měření';
                }

                const spEl = document.getElementById('measureSessionPath');
                if (spEl && stateData.session_path_raw) {
                    const solved = stateData.session_path_solved || '-';
                    spEl.textContent = 'CSV: ' + stateData.session_path_raw;
                    spEl.title = 'RAW: ' + stateData.session_path_raw + '\nSOLVED: ' + solved;
                }
                ...
                // Update webcam-detected QR code in the Měření page.
                const detectedEl = document.getElementById('webcamQrDetected');
                const previousEl = document.getElementById('webcamQrPrevious');
                const webcamBtn = document.getElementById('webcamQrBtn');
                if (scanData.qr_detected_code) {
                    lastWebcamQrCode = scanData.qr_detected_code;
                    detectedEl.innerHTML = `Naskenováno: <strong>${scanData.qr_detected_code}</strong>`;
                    webcamBtn.disabled = false;
                } else {
                    detectedEl.textContent = 'Naskenujte QR kód webkamerou';
                    // Keep the button enabled if we already have a code from earlier.
                    webcamBtn.disabled = !lastWebcamQrCode;
                }
                if (scanData.qr_previous_code) {
                    webcamPreviousCode = scanData.qr_previous_code;
                }
                previousEl.textContent = scanData.qr_previous_code
                    ? `Předchozí: ${scanData.qr_previous_code}`
                    : '';
...
def cal_point():
    ...
    def finish():
        ...
        log(f"Calibration point {cal3d['point_idx']} finished after {elapsed_ms/1000:.2f}s")
...
@app.route("/qr_detect", methods=["POST"])
def qr_detect():
    """Receive a QR code detected by the in-browser webcam scanner.

    Unlike /manual_qr, this only updates the detected-code state; the user
    still has to click the start button to begin sampling.
    """
    global qr_detected_code, qr_previous_code, qr_detected_ms, qr_last_seen_ms
    data = request.get_json(silent=True) or {}
    qr_code = str(data.get("qr", "")).strip()
    if not qr_code:
        return jsonify({"error": "Missing qr"}), 400
    ...
    qr_detected_code = qr_code
    qr_detected_ms = time.time() * 1000
    log(f"[QR_DETECT] browser detected {qr_code}")
    return jsonify({"ok": True})
...
def scan_state():
    ...
    with transform_lock:
        return jsonify({
            ...
            "qr_detected_code": qr_detected_code,
            "qr_previous_code": qr_previous_code,
            "trny": trny,
            "pairs": [
                {"point_id": p["point_id"], "uwb": list(p["uwb"]), "global": list(p["global"])}
                for p in transform["pairs"]
...
def main():
    ...
    load_trny()
    load_transform()
    # Create or reuse a session CSV pair immediately on startup.
    # If the most recent session is less than 2 hours old, append to it;
    # otherwise start a fresh session. This way a restart within 2 hours
    # continues the same CSV, and a later restart starts a new one.
    ensure_session()
    threading.Thread(target=udp_listener, daemon=True).start()
    threading.Thread(target=qr_listener, daemon=True).start()
    https_ok = start_https_server()
    ip = get_pc_ip()
    ...
    if https_ok:
        print(f"  HTTPS (remote devices with camera): https://{ip}:{HTTPS_PORT}")
        print("  Remote devices must accept the self-signed certificate")
        print("  warning once, then the camera works there too.")
    print("=" * 60)
    # Open the default browser via 127.0.0.1 so the browser webcam API
    # (getUserMedia) works without HTTPS. Remote clients can still use the IP URL.
